...
export default function TicketPrompt() {
  ...
  useEffect(() => {
    ...
    fetch('/api/links', { cache: 'no-store' })
      ...
      .then(({ links }) => {
        const popup = links.find((item) => item.showTicketPrompt)
        if (!active || !popup) return
        try {
          if (sessionStorage.getItem(`tmucsa:ticket-prompt:${popup.id}`)) return
        } catch {}
        setLink(popup)
        timeout = window.setTimeout(() => {
          if (!active) return
          setVisible(true)
          trackAnalytics({ type: 'prompt_view', linkId: popup.id })
        }, 1200)
      })
      .catch(() => {})
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-end justify-center bg-black/55 p-4 backdrop-blur-sm sm:items-center" role="dialog" aria-modal="true" aria-labelledby="ticket-prompt-title">
      <section className="relative w-full max-w-lg overflow-hidden border border-white/15 bg-[#161329] p-6 text-white shadow-2xl sm:p-8">
        <button type="button" onClick={close} className="absolute right-4 top-4 flex h-10 w-10 items-center justify-center border border-white/15 text-white/55 transition hover:border-white/40 hover:text-white" aria-label="Close popup">
        ...
        <p className="font-jost text-[10px] font-semibold uppercase tracking-[0.28em] text-beige/55">{link.category === 'Tickets' ? 'Upcoming event' : link.category || 'Featured link'}</p>
        <h2 id="ticket-prompt-title" className="mt-5 pr-10 font-josefin text-3xl font-semibold sm:text-4xl">{link.title}</h2>
        {link.description ? <p className="mt-3 font-jost font-light leading-6 text-white/55">{link.description}</p> : null}
        <div className="mt-7 flex flex-col gap-3 sm:flex-row">
          <a href={link.url} target="_blank" rel="noopener noreferrer" onClick={() => { trackAnalytics({ type: 'prompt_click', linkId: link.id }); close() }} className="inline-flex flex-1 items-center justify-center gap-2 bg-beige px-5 py-3.5 font-jost text-sm font-medium text-[#161329] transition hover:bg-white">
            {link.popupButtonText || (link.category === 'Tickets' ? 'Get tickets' : 'Learn more')}
